...
describe("Block", () => {
  describe("calculateBlockTargetHash()", () => {
    ...
    it("calculates the low hash value when the last block difficulty is high", () => {
      expect(
        Block.calculateBlockTargetHash({
  ...
  describe("validateBlock()", () => {
    ...
    it("rejects when the proof of work requirement is not met", () => {
      const original = Block.calculateBlockTargetHash;
      Block.calculateBlockTargetHash = () => {
        return "0".repeat(64);
      };
      ...
      Block.calculateBlockTargetHash = original;
    });
  });
});
